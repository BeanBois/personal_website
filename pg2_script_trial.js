...
Array.from(uiContainer.children).forEach((uiElement) => {
    ...
    if(uiElement.value && uiElement.className === 'exit'){
...
let initXUIRefPoint = defaultSpawnPoint.x; //490 left
let initYUIRefPoint =  defaultSpawnPoint.y;//2470 top
    ...
    backgroundImage.onload = () => {
        collisionMapImage.onload = () => {
            ...
            function drawBackground() {
                ...
                const zoomedCenterX = xUIRefPoint + playerWidth /2; //left
                const zoomedCenterY = yUIRefPoint - playerHeight/2; //top
                
            
                // Calculate the draw position on the canvas to center the zoomed background image
                const drawX = canvas.width / 2 - zoomedCenterX; //left
                const drawY = canvas.height / 2 - zoomedCenterY; //top
            ...
            function hit_Exit(x,y, proximityRange = collisionProximity ){
                ...
                if (exits.length === 0) {
                    return {exit : false, id : null}; // No UI elements to check for collision with
                }
                    ...
                    switch (currentOrientation) {
                        case 'up':
                          ...
                          if (playerT - proximity <= uiBottom && playerT >= uiBottom && playerL >= uiLeft && playerR <= uiRight) {
                            return {exit: true, id : id}; // Collision detected
                          }
                          break; 
                        case 'down':
                        ...
                        case 'left':
                            console.log('player : ', playerT, playerL, playerB, playerR);
                            console.log('exit : ', uiTop, uiLeft,uiBottom,uiRight);
            ...
            window.addEventListener("keydown", async (event) => {
                ...
                //handle exit
                const item = hit_Exit(playerX, playerY);
                const exit = item['exit'];
                if(exit){
                    const exitUI = document.getElementById(item['id']);
                    
                    if(item.id=="exit3" || item.id=='exit'){
                        window.location.href = exitUI.value;
                    }
                    else alert(`area not developed yet! Sorry!`);
                    return
